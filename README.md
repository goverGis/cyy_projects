...
> **项目根目录**：`E:\就业\项目\trae`（Windows 本机路径）
...
| 脚本 | 作用 |
|---|---|
| `run.bat` | 开两个独立窗口拉起后端 `:8000` + 前端 `:3000`，等 8 秒自动打开浏览器 |
| `stop.bat` | 按端口（8000 / 3000 / 3001 / 3002）结束两个服务 |

`run.bat` 会先自检 `api/.venv/Scripts/python.exe` 与 `frontend/node_modules`，缺失时给出明确提示并停住窗口，不会一闪而过。

若后端窗口报连接错误，多半是 PostgreSQL 服务没起 —— 到 Windows「服务」里启动 `postgresql-x64-13`，再重跑 `run.bat`。

> ⚠️ **维护这两个脚本时务必注意**：`.bat` 文件必须是 **CRLF 换行 + 纯 ASCII 内容**。
> 用纯 LF 换行会让 cmd.exe 解析 `if (...)` 块和 `goto` 标签时错乱、窗口一闪就关；
> 在含中文的 `.bat` 里写 `chcp 65001` 也会导致 cmd 中途切换代码页后错位。
> 这两个坑都踩过了，所以脚本刻意只用英文提示。
